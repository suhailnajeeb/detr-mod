...
@torch.no_grad()
def evaluate_nms(model, criterion, postprocessors, data_loader, base_ds, device, output_dir):
    model.eval()
    criterion.eval()
    ...
    for samples, targets in metric_logger.log_every(data_loader, 10, header):
        ...
        for result in results:
            ...
            indices = nms(result['boxes'], result['scores'], iou_threshold=0.5)
            result['scores'] = result['scores'][indices]
            result['labels'] = result['labels'][indices]
