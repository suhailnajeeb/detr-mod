...
python eval_and_time.py --dataset "coco"\
  --coco_path "/data/gpfs/projects/punim1800/RDD-2022/holdout/coco_holdout" \
  --output_dir "outputs/holdout_00/"\
  --batch_size 16\
  --num_workers 1
...
from engine import evaluate_and_save, evaluate_and_time
...
if __name__ == "__main__":
    ...
    PATHS = {
        ...
        "test": (root / "test2017", root / "annotations" / f'{mode}_test2017.json'), # additional line for test set evaluation
    }

    img_folder, ann_file = PATHS["test"]

    dataset = CocoDetection(img_folder, ann_file, transforms=make_coco_transforms(image_set = 'test'), return_masks=args.masks)
    ...
    test_stats, coco_evaluator, avg_time = evaluate_and_time(model, criterion, postprocessors, \
        data_loader, base_ds, device, args.output_dir)

    print('Average Inference Time: {:3f} ms'.format(avg_time*1000))